...
export default function SkillsPageClient({ skillPackages, skillsRegistry, runes }: SkillsPageClientProps) {
  ...
  // ── Filtered actions ─────────────────────────────────────────────────────
  const filteredActions = skillsRegistry.filter(s => {
    ...
    <div className="rg-page" style={{ maxWidth: '1300px', margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: '2rem' }}>
        ...
        <p style={{ color: '#8ab4e0', fontSize: '0.95rem', margin: 0 }}>
          {skillPackages.length} skills · {skillsRegistry.length} actions · powering {runes.length} runes
        </p>
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.5rem', flexWrap: 'wrap' }}>
        ...
        <div style={{
          ...
          {[
            { id: 'skills',  label: `Skills  ${skillPackages.length}` },
            { id: 'actions', label: `Actions  ${skillsRegistry.length}` },
          ].map(tab => (
            <button
              key={tab.id}
              ...
              return (
                <button key={cat} onClick={() => setActionCat(cat)} style={{
                  ...
                  <span style={{ marginLeft: '4px', fontSize: '0.7rem', opacity: 0.65 }}>
                    {cat === 'all' ? skillsRegistry.length : skillsRegistry.filter(s => s.category === cat).length}
                  </span>
                </button>
              )
            })}
                ...
                <div style={{
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
                    {pkg.actionIds.slice(0, 6).map(aid => {
                      const action = skillsRegistry.find(s => s.id === aid)
      ...
      {view === 'actions' && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(270px, 1fr))', gap: '0.75rem' }}>
          {filteredActions.map(action => {
            ...
            const parentSkill = skillPackages.find(p => p.actionIds.includes(action.id))
            return (
