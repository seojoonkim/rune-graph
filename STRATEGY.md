...
**도메인:** `runegraph.com` ✅ (가용 확인 — `.com` `.ai` `.io` 전부 가용)

> `.com` 확보 가능 — 이전 모든 후보(runedex, runehub, runestore 등)는 `.ai`만 가용이었음. 이것만으로도 RuneGraph는 차원이 다른 선택.
...
RuneGraph     = 검증된 Rune의 관계 그래프 마켓플레이스
```

**포지셔닝:**
> "ClawHub이 npm이라면, RuneGraph는 Verified App Store + 스킬 관계 지도"

### 네이밍 구조

```
RuneGraph   = 마켓플레이스 (발견·탐색·거래)
...
> "npm은 재료를 파는 곳이에요. RuneGraph는 레시피를 공유하고, 모든 재료의 관계를 지도로 보여주는 곳이에요."
...
| 9 | RuneGraph vs GPT Store vs ClawHub |
...
| 차원 | ClawHub | GPT Store | **RuneGraph** |
|------|---------|-----------|---------------|
...
2. **스킬 관계 코퍼스** — Rune 100개 = 수천 조합, 조합 검증 데이터는 RuneGraph만 보유
...
| # | 액션 | 소요 |
|---|------|------|
| **1** | **`runegraph.com` 도메인 확보** | 30분 🔴 긴급 |
| **2** | **GitHub `rune-graph` 레포 생성 + Next.js 초기화** | 1시간 |
| **3** | **10개 데모 Rune 스펙 작성 + 그래프 JSON** | 1일 |
| **4** | **React Flow 기반 그래프 뷰어 프로토타입** | 3일 |
| **5** | **Vercel 배포 + runegraph.com 연결** | 1시간 |

---

*RuneGraph — Inscribe. Invoke. Trust. 🔮*
